...
TEST(BitSetTest, InvalidCases) {
    ...
    const uint32_t original = bits.get_base_storage();

    bits.set_range(0, 32, 15);
    EXPECT_EQ(bits.get_base_storage(), original);
...
TEST(BitSetTest, ValidEdgeCases) {
    ...
    EXPECT_EQ(bits.get_base_storage(), 0xFFFFFFFF);

    bits.set_range(0, 7, 0);
    EXPECT_EQ(bits.get_range(0, 7), 0);

    bits.set_range(0, 31, 0);
    EXPECT_EQ(bits.get_base_storage(), 0);
}
