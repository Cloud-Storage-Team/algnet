...
public:
    ...
    virtual std::vector<std::shared_ptr<IRoutingDevice>> get_neighbours() const final;
    virtual std::shared_ptr<sim::ILink> get_link_to_device(std::shared_ptr<IRoutingDevice> device) const final;
};
