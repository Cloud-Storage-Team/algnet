...
SizeByte FlowMock::get_sending_quota() const { return m_stat.sending_quota; }
TimeNs FlowMock::get_last_rtt() const { return m_stat.last_rtt; }
void FlowMock::send_data(SizeByte data) {
    if (data > m_stat.sending_quota) {
        throw std::runtime_error("FlowMock::send_data: data > sending_quota");
    }
    m_stat.sending_quota -= data;
}
...
void FlowMock::set_flow_stat(FlowStat stat) { m_stat = stat; }
