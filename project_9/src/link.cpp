...
Link::Link(std::shared_ptr<NetworkElement> &source,
           std::shared_ptr<NetworkElement> &destination, std::uint64_t speed,
           std::uint64_t last_process_time_ns)
    : source(source), destination(destination), speed(speed),
      last_process_time_ns(last_process_time_ns) {
}

std::shared_ptr<NetworkElement>
Link::GetNextElement(std::uint64_t destination_id = 0) {
    return destination;
}

void Link::ReceivePacket(std::uint64_t current_time_ns, PacketHeader &packet,
