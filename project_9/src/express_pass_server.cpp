...
ExpressPassReciever::SendPackets(std::uint64_t current_time_ns,
    ...
    for (auto destination_id : senders_ids) {
        ...
        PacketHeader credit =
            congestion_control.GetCredit(sending_time, id, destination_id);
        packets_wrapped.push(
            RoutingPacket(credit, this->GetNextElement(destination_id)));
    }
...
void ExpressPassSender::ReceivePacket(std::uint64_t current_time_ns,
    ...
    if (packet.GetFlag(2) == 1) {
        ...
        PacketHeader data = congestion_control.GetDataPacket(
            current_time_ns + process_time_ns, id, packet.GetSourceID());
        packets_wrapped.push(
