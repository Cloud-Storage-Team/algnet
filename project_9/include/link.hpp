...
class Link : public NetworkElement {
  ...
  public:
    Link() = default;
    Link(std::shared_ptr<NetworkElement> &source,
    ...
    // Updates last_process_time_ns, sets new sending_time for packet and
    // puts packet back to queue
...
struct Connection {
    ...
    Connection(std::shared_ptr<Link> &&direct_link,
