...
public:
    ...
    /**
     * Processes the obtained packet (basicly adds processing time to packet's sending time)
     *
...
public:
    ...
    RoutingNetworkElement(std::unordered_map<std::uint64_t, std::shared_ptr<NetworkElement>>& routing_table);
    void AddNewRout(std::uint64_t destination_id, std::shared_ptr<NetworkElement>& link);
