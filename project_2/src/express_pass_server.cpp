...
std::uint64_t ExpressPassReceiver::SendCreditByFlow(std::uint64_t flow_id, std::uint64_t current_time_ns, EventQueue& all_events) {
    ...
    auto process = [this, flow_id, sending_time, credit](EventQueue& events) {
    ...
    if (next_sending < simulation_duration) {
        auto create_credit = [this, flow_id, next_sending](EventQueue& events) {
...
std::uint64_t ExpressPassReceiver::SendPackets(std::uint64_t current_time_ns, EventQueue& all_events) {
...
void ExpressPassReceiver::ReceivePacket(std::uint64_t current_time_ns, PacketHeader& packet, EventQueue& all_events) {
...
std::uint64_t ExpressPassSender::SendPackets(std::uint64_t current_time_ns, EventQueue& all_events) {
    // if (current_time_ns == 0) {
...
void ExpressPassSender::ReceivePacket(std::uint64_t current_time_ns, PacketHeader& packet, EventQueue& all_events) {
    if (packet.GetFlag(PacketType::IsInitializer) == 1) {
        ...
        auto process = [this, current_time_ns, process_time, data](EventQueue& events) {
