...
PacketHeader ExpressPass::GetCredit(std::uint64_t sending_time, std::uint64_t source_id, std::uint64_t destination_id, std::uint64_t size) const {
    ...
    PacketHeader credit = PacketHeader(source_id, destination_id, sending_time, 0, size);
    credit.SetFlag(0, 1);
    return credit;
}
...
PacketHeader ExpressPass::GetHandshakePacket(std::uint64_t sending_time, std::uint64_t source_id, std::uint64_t destination_id, std::uint64_t size) const {
    PacketHeader handshake = PacketHeader(source_id, destination_id, sending_time, 0, size);
    handshake.SetFlag(2, 1);
    return handshake;
}

PacketHeader ExpressPass::GetDataPacket(std::uint64_t sending_time, std::uint64_t source_id, std::uint64_t destination_id, std::uint64_t size) const {
    PacketHeader data = PacketHeader(source_id, destination_id, sending_time, 0, size);
    return data;
}