...
NetworkSimulator::NetworkSimulator(
    ...
    : receiver(receiver), n_switch_1(n_switch_1), n_switch_2(n_switch_2),
      simulation_duration_ns(simulation_duration_ns) {
        ...
        // std::endl;
        routing_switch_1->AddNewRout(receiver->GetID(),
                                     recv_switch_1_conn.reversed_link);
        routing_switch_2->AddNewRout(receiver->GetID(),
                                     switch_1_2_conn.reversed_link);

        for (std::shared_ptr<ServerBase> sender : senders) {
                ids_to_senders[sender->GetID()] = sender;
                std::shared_ptr<RoutingNetworkElement> routing_sender =
                    std::dynamic_pointer_cast<RoutingNetworkElement>(sender);
                auto switch_sender_conn =
                    AddNewConnection(routing_switch_2, routing_sender, 5);
                routing_switch_2->AddNewRout(sender->GetID(),
                                             switch_sender_conn.direct_link);
                routing_switch_1->AddNewRout(sender->GetID(),
                                             switch_1_2_conn.direct_link);
                routing_receiver->AddNewRout(sender->GetID(),
                                             recv_switch_1_conn.direct_link);
                routing_sender->AddNewRout(receiver->GetID(),
                                           switch_sender_conn.reversed_link);
        }
}

Connection NetworkSimulator::AddNewConnection(
    std::shared_ptr<RoutingNetworkElement> source,
    std::shared_ptr<RoutingNetworkElement> destination, std::uint64_t speed) {
        std::shared_ptr<NetworkElement> base_source =
            std::dynamic_pointer_cast<NetworkElement>(source);
        std::shared_ptr<NetworkElement> base_destination =
            std::dynamic_pointer_cast<NetworkElement>(destination);
        std::shared_ptr<Link> link =
            std::make_shared<Link>(Link(base_source, base_destination, speed));
        std::shared_ptr<Link> reverse_link =
            std::make_shared<Link>(Link(base_destination, base_source, speed));
...
void NetworkSimulator::SendPackets(PriorityQueueWrapper &wrapped_packets) {
        ...
        std::uint64_t possible_next_ask =
            receiver->SendPackets(current_time_ns, wrapped_packets);
        next_ask =
            std::min(next_ask,
                     ((possible_next_ask == 0) ? next_ask : possible_next_ask));

        possible_next_ask =
            n_switch_1->SendPackets(current_time_ns, wrapped_packets);
        next_ask =
            std::min(next_ask,
                     ((possible_next_ask == 0) ? next_ask : possible_next_ask));

        possible_next_ask =
            n_switch_2->SendPackets(current_time_ns, wrapped_packets);
        next_ask =
            std::min(next_ask,
                     ((possible_next_ask == 0) ? next_ask : possible_next_ask));

        for (auto &id_and_sender : ids_to_senders) {
                possible_next_ask = id_and_sender.second->SendPackets(
                    current_time_ns, wrapped_packets);
                next_ask = std::min(
                    next_ask,
                    ((possible_next_ask == 0) ? next_ask : possible_next_ask));
        }
...
void NetworkSimulator::ProcessNextPacket(
    PriorityQueueWrapper &wrapped_packets) {
        // std::cout << "Processed pac" << std::endl;
        RoutingPacket r_packet = packets.top();
        // std::cout << "Removed: " << packet.GetDestinationID() << " " <<
        // packet.GetSendingTime() << std::endl;
        packets.pop();
        PacketHeader packet = r_packet.GetPacket();
        // std::cout << "Removed: " << packet << std::endl;
        r_packet.GetNext()->ReceivePacket(current_time_ns, packet,
                                          wrapped_packets);
}
...
void NetworkSimulator::StartSimulation() {
        ...
        while (current_time_ns <= simulation_duration_ns) {
                ...
                std::uint64_t next_packet_time =
                    packets.top().GetPacket().GetSendingTime();
