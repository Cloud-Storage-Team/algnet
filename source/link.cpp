...
Link::Link(std::weak_ptr<IRoutingDevice> a_from,
    ...
    if (a_from.expired() || a_to.expired()) {
        Logger::WARN("Passed link to device is expired");
    } else if (a_speed_mbps == 0) {
        Logger::WARN("Passed zero link speed");
    }
}
...
void Link::schedule_arrival(Packet packet) {
    if (m_to.expired()) {
        Logger::WARN("Destination device pointer is expired");
        return;
    }

    Logger::INFO("Packet arrived to link's ingress queue. Packet: " + packet.to_string());
...
void Link::process_arrival(Packet packet) {
    Logger::INFO("Packet arrived to link's egress queue. Packet: " + packet.to_string());
...
std::optional<Packet> Link::get_packet() {
    if (m_next_ingress.empty()) {
        Logger::INFO("Ingress packet queue is empty");
        return {};
    }

    auto packet = m_next_ingress.front();
    Logger::INFO("Taken packet from link. Packet: " + packet.to_string());
    m_next_ingress.pop();
    return packet;
};
...
std::shared_ptr<IRoutingDevice> Link::get_from() const {
    if (m_from.expired()) {
        Logger::WARN("Source device pointer is expired");
        return nullptr;
    }
