...
void YamlParser::parse_topology_config(const std::string &filename,
    ...
    process_hosts(config, simulator);
    process_switches(config, simulator);
    process_links(config, simulator);
}

void YamlParser::parse_simulation_config(const std::string &filename,
                                         Simulator &simulator) {
    const YAML::Node config = YAML::LoadFile(filename);
...
void YamlParser::process_hosts(const YAML::Node &config, Simulator &simulator) {
    if (!config["hosts"]) {
        LOG_INFO("No hosts in topology config");
        return;
    }
...
void YamlParser::process_switches(const YAML::Node &config,
                                  Simulator &simulator) {
    if (!config["switches"]) {
        LOG_INFO("No switches in topology config");
        return;
    }
...
void YamlParser::process_links(const YAML::Node &config,
    ...
    if (!config["links"]) {
        LOG_INFO("No links in topology config");
        return;
    }
...
void YamlParser::process_flows(const YAML::Node &config,
                               Simulator &simulator) const {
    if (!config["flows"]) {
        LOG_INFO("No flows in simulation config");
        return;
    }
