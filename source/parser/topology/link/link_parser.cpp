...
std::shared_ptr<Link> LinkParser::parse_link(
    const ConfigNodeWithPreset& link_node) {
        ...
        SpeedGbps speed = parse_speed(link_node["throughput"].value_or_throw().get_node());
        TimeNs delay = parse_time(link_node["latency"].value_or_throw().get_node());
        SizeByte egress_buffer_size = parse_size(link_node["egress_buffer_size"].value_or_throw().get_node());
        SizeByte ingress_buffer_size = parse_size(link_node["ingress_buffer_size"].value_or_throw().get_node());
        return std::make_shared<Link>(id, from_ptr, to_ptr, speed, delay, egress_buffer_size, ingress_buffer_size);
}
