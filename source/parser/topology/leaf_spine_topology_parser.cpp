...
Topology parse_leaf_spine_topology(const ConfigNodeWithPreset& node) {
    ...
    TopologyContext ctx = create_leaf_spine_topology_context(params);
...
TopologyContext create_leaf_spine_topology_context(
    ...
    std::vector<std::shared_ptr<ISwitch>> spine_switches =
        create_spine_switches(params.spine_switches, params.packet_spraying,
                              names_prefix);

    std::vector<std::shared_ptr<ISwitch>> leaf_switches = create_leaf_switches(
        params.leaf_switches, params.packet_spraying, names_prefix);

    std::vector<std::shared_ptr<IHost>> hosts =
        create_hosts(params.hosts, leaf_switches.size(), names_prefix);

    utils::IdTable<ILink> links_table =
        create_links(spine_switches, leaf_switches, hosts,
                     params.host_leaf_links, params.leaf_spine_links);

    utils::IdTable<IHost> hosts_table;
    for (auto host : hosts) {
        hosts_table.add(host);
    }

    utils::IdTable<ISwitch> switches_table;
    for (auto swtch : leaf_switches) {
        switches_table.add(swtch);
    }
    for (auto swtch : spine_switches) {
        switches_table.add(swtch);
    }

    return TopologyContext{hosts_table, switches_table, links_table};
}
