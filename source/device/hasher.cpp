...
std::uint32_t RandomHasher::get_hash(Packet, Id) const {
    return static_cast<std::uint32_t>(std::rand());
};

std::uint32_t BaseHasher::get_hash(Packet packet, Id) const {
...
std::uint32_t SymmetricHasher::get_hash(Packet packet, Id) const {
    std::hash<std::string> hasher;
