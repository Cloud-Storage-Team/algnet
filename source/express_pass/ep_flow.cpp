...
EPFlow::EPFlow(std::shared_ptr<ISender> a_src, std::shared_ptr<IReceiver> a_dest,
    ...
    : m_sender_status(SenderStatus::CREDIT_STOP_S),
      ...
      m_id(IdentifierFactory::get_instance().generate_id()) {
        ...
        if (auto src = std::dynamic_pointer_cast<EPSender>(a_src)) {
            m_src = src;
        } else {
            throw std::invalid_argument("Sender must be of type EPSender");
        }

        if (auto dest = std::dynamic_pointer_cast<EPReceiver>(a_dest)) {
            m_dest = dest;
        } else {
            throw std::invalid_argument("Receiver must be of type EPReceiver");
        }
