...
std::shared_ptr<ILink> ExpressPassSender::get_link_to_destination(
...
Time ExpressPassSender::process(Time current_time) {
    ...
    if (packet.type == PacketType::ACK && destination.get() == this) {
        ...
        LOG_WARN(
            "Packet arrived to ExpressPassSender that is not its destination; use routing "
...
std::set<std::shared_ptr<ILink>> ExpressPassSender::get_outlinks() const {
