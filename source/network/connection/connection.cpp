...
void Connection::send_new_portion() {
    while (!m_sending_queue.empty()) {
        ...
        OnDeliveryCallback callback = [connection, id, delivery_size,
                                       total_delivered_on_send, send_time]() {
            connection->process_data_delivery(
                id, delivery_size, total_delivered_on_send, send_time);
        };
...
void Connection::process_data_delivery(DataId id, SizeByte delivery_size,
    ...
    if (it != m_data_context_table.end()) {
        ...
        LOG_ERROR(fmt::format(
            "Connection {}: could not find context for data with id {}", m_id,
            id.to_string()));
    }
    send_new_portion();
}
